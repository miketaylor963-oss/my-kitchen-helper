...
|derived-non-contiguous-steps|pass|non-contiguous step\_indices \[0,3,5]|
...
|advisory-consistency-trip|pass (advisory silent)|declared vegan with butter/cream and prep-noted ingredient names that don't all exact-match the seed; tests advisory's silent path when matching is incomplete (rule 204's preconditions unmet)|
|advisory-consistency-trip-fires|pass (advisory fires)|declared vegan with butter and double cream as canonical names that exact-match seeded vegetarian-classified ingredients; tests advisory's firing path (rule 204's preconditions met, declared category less restrictive than strictest matched ingredient)|
|derived-unsorted-duplicated-step-indices|reject|duplicate step\_indices (1 appears twice)|
|nonstandard-ingredient-ids|reject|ids don't match ^\[0-9]{4}$|
|derived-slug-collides-with-parent|reject|derived external\_ref equals parent's|
|empty-ingredients-array|reject|empty ingredients array|
|empty-steps-array|reject|empty steps array|
|derived-empty-arrays|reject|derived ingredient\_ids and step\_indices empty|

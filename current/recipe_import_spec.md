...
**Optional ingredients.** Include as full ingredient rows with `notes: "optional, ..."` describing when they're used. Don't drop them and don't invent a new field. Method-only optionals — where the method mentions an ingredient as conditional ("add a tablespoon of oats if the mixture is very wet") and the ingredient doesn't appear in the source's ingredient list — still get a full ingredient row, with `amount: null` / `unit: null` and `notes: "optional; <condition from method>"`. This keeps the ingredient ID available for step references and surfaces the conditional to the operator at import time.

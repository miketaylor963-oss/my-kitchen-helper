...
## Strip-list extension slice

**Date:** 2026-06-21.

Three mechanisms added to `src/lib/import/strip_list.ts` and `generateVariants` in `src/lib/import/matching.ts`.

---

### Step 1 — Trailing clause extension

**Previous limitation:** `generateVariants` required every token in the trailing comma-clause to be in `prep_verbs ∪ modifying_adverbs`. Connectives (`and`), prepositions (`into`, `on`, `the`), cut-shape nouns (`dice`, `slices`), cut-direction words (`diagonal`), and dimension tokens (`1.5cm`) all blocked the strip.

**Mechanism change:** Two-condition check replaces the single-condition check:
1. At least one token must be in `allPrep` (`prep_verbs ∪ modifying_adverbs`) — the "anchor" requirement that prevents pure-connective clauses stripping.
2. Every token must be in `allAllowed` or match the dimension pattern `/^\d+(\.\d+)?(cm|mm|g|kg|ml|l|inch|inches)$/i`.

`allAllowed` = `allPrep ∪ connectives ∪ prepositions ∪ cut_descriptors`.

**New `StripList` fields:**
- `prep_verbs` additions: `cut`, `chunked`
- `modifying_adverbs` addition: `very`
- `connectives`: `['and', 'then']`
- `prepositions`: `['into', 'on', 'the']`
- `cut_descriptors`: `['dice', 'slices', 'diagonal']` — only evidenced terms; `wedges`, `rounds`, `strips` not yet in data

**Clauses now stripping (confirmed against test suite):**
- `drained and rinsed` (3 fixtures) — `and` connective
- `peeled and chunked` (2 fixtures) — `and` connective + `chunked` prep_verb
- `cut into 1.5cm dice` (`courgettes`) — `cut` prep_verb + `into` preposition + dimension + `dice` cut_descriptor
- `cut into diagonal slices` (`carrot`) — `cut` + `into` + `diagonal` + `slices` cut_descriptors
- `very finely sliced` (`spring onion greens`) — `very` modifying_adverb; trailing strip now fires; base `spring onion greens` still fuzzy to canonical `spring onion`

**Clauses correctly NOT stripping:**
- `rind grated and juiced` — `rind` is a food-part noun; `none` by operator override is correct steady-state
- `whites sliced on the diagonal` — `whites` is a food-part noun; same reasoning

**`courgettes` importer-fix carry-forward resolved:** Step 1 strips `cut into 1.5cm dice` to `courgettes`; Step 2 (below) singularises to `courgette` → exact canonical match. The alias-via-fuzzy 0.37 path is superseded.

---

### Step 2 — Pluralisation

**Mechanism:** `toSingular(name)` helper generates a de-pluralised variant from the last word of a name. Applied to each variant in a snapshot of the variants array (no recursion). Rules:
- Last word ends in `oes` (length > 3) → strip `es`: `potatoes` → `potato`
- Last word ends in `s`, not `ss`, not `us`, length > 2 → strip `s`: `cloves` → `clove`
- The `!endsWith("us")` guard explicitly excludes `-us`/`-ous` words (`houmous`, `couscous`, `asparagus`) that are not English plurals

**New variants confirmed:**
- `garlic cloves` (after strip of `, [prep]`) → `garlic clove` → exact canonical
- `celery stalks` (after strip) → `celery stalk` → exact canonical
- `carrots` (after strip) → `carrot` → exact canonical
- `eggs` (after strip of `, beaten`) → `egg` → exact canonical
- `sweet potatoes` (after Step 1 strip) → `sweet potato` → exact canonical
- `courgettes` (after Step 1 strip) → `courgette` → exact canonical

**Not improved by pluralisation alone:**
- `garlic cloves, minced` original → last word `minced` (not plural) → null; singular applied to stripped `garlic cloves` instead ✓
- `tamago eggs` (after strip of `, halved`) → singular → `tamago egg`; DB query for `tamago egg` produces lower-score fuzzy than `tamago eggs`; fuzzy merge keeps best score → outcome unchanged ✓
- `classic houmous` → `houmous` last char is `s` but ends in `us` → excluded → no `classic houmou` variant generated ✓

---

### Step 3 — Quality/sourcing qualifier leading strip

**Mechanism:** Leading strip extended to check `quality_qualifiers` in addition to `size_adjectives`. The "both-strip" path receives the same extension. `quality_qualifiers` = `['free-range', 'organic', 'grass-fed']` — confirmed carry-forward terms only.

**New variant confirmed:**
- `free-range eggs` → strips `free-range` → `eggs` → singular → `egg` → exact canonical

---

### Tests

14 cases added to `src/lib/import/matching.test.ts` under `describe('generateVariants', ...)`. All 18 tests passing (14 new + 4 existing advisory tests).

---

### Step 5 — Carry-forward routing

| Item | Routes to | This slice |
|---|---|---|
| `drained and rinsed` (3 fixtures) | strip-list | Fixed — Step 1 |
| `peeled and chunked` (2 fixtures) | strip-list | Fixed — Step 1 |
| `cut into 1.5cm dice` (`courgettes`) | strip-list | Fixed — Step 1 + Step 2 (closes importer-fix carry-forward) |
| `cut into diagonal slices` (`carrot`) | strip-list | Fixed — Step 1 |
| `very finely sliced` (`spring onion greens`) | strip-list | Fixed — `very` in modifying_adverbs; base `spring onion greens` still fuzzy |
| Pluralisation gap (garlic cloves, celery stalks, carrots, eggs, sweet potatoes) | strip-list | Fixed — Step 2 |
| `free-range eggs` | strip-list | Fixed — Step 3 + Step 2 |
| `lemons, rind grated and juiced` | accepted as-is | Food-part noun `rind` blocks; `none` by operator override is correct |
| `spring onions, whites sliced on the diagonal` | accepted as-is | Food-part noun `whites` blocks |
| `tinned X` convention (4 fixtures) | converter-prompt | Not this slice |
| `nutmeg, grated` not `grated nutmeg` | converter-prompt | Not this slice |
| Mid-name modifiers (rice wine vinegar, dried shiitake, fresh root ginger, tinned chopped tomatoes, rose harissa, miso paste, red split lentils, fresh spinach, bare coriander) | converter-prompt | Not this slice |
| X-or-Y alternative forms (`green or brown lentils`) | converter-prompt | Not this slice |
| `garlic cloves` plural alias | master vocabulary | Not this slice — Mike adds via SQL/admin |
| `suet` vs `vegetarian suet` | content decision / master vocab | Not this slice |
| `salami`/`diced lamb`/`chicken breast` false positives | threshold tuning | Not this slice — held at 0.30 |

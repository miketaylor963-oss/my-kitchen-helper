...
**Status (F2C complete):** Recipe-shape import (modes 1 and 2) operational end-to-end. JSON paste → validate (with existing-recipe advisory) → match ingredients (human-in-the-loop, with prep-adjective stripping) → commit via `commit_import` RPC. Re-import uses `on_conflict='update'`; requires slug-match between the fixture `external_ref` and the existing row — canonical slug convention codified in `current/recipe_import_spec.md` and the dedicated converter project. F2C content coverage complete: all twelve web_sourced recipe-shape fixtures committed via upsert-replace (five in 2C.3; remainder in the post-2C.4 re-import sweep, 2026-06-21). Decision 57 item 3 closed. Dietary-consistency advisory banner now evaluates fuzzy rows (advisory gate fix, 2026-06-21; previously suppressed by `allExact` gate). Unit test coverage added via vitest. In-app Claude call (mode 3) remains outstanding.

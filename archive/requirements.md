# Recipe & Meal Planning App — Requirements (Draft v0.3)
...
### 3.9 Recipe Import from URLs (future)
...
### 5.1 Tech Stack
- **Builder:** Lovable (primary), with Claude Code as a fallback for complex features once Lovable's context breaks down.
- **Database:** Supabase Postgres (~26 tables).
- **Schema installation:** done directly via Supabase SQL editor, in dependency order — *not* via Lovable, to avoid drift.
- **Version control:** GitHub, via Lovable's sync integration. Commit after each working chunk.
...
- **Allowed writers** are a hardcoded (or admin-managed) list — Mike and Jane to start. Adding a writer is a manual operation, not a self-serve signup.
- **Supabase RLS:** read policies open; write policies restricted to authenticated users in the writers list.
...
Add a `household_id` (or equivalent owner reference) to every writable table — meals, components, ingredients, meal plan entries, cooked log, shopping list, etc. v1 has one household; the column is functionally unused. Cost now: one column per table. Cost later: migrating ~26 tables and rewriting every query. Keeps the door open to multi-household without committing to it.

### 6.2 Writers as a database table
Define an `app_writers` (or similar) table from day one rather than hardcoding the writer list in code or RLS policies. Flipping to invite-based access later becomes a row insert, not a code change.

### 6.3 Public read — what's actually public
Currently "read access unrestricted" is the broad answer. Before writing RLS, decide per-table:
- **Library (meals, components, ingredients, frameworks, lookup tables):** public read. This is the portfolio asset.
- **Meal plan entries:** ? — feels more personal. Default to public unless a reason to restrict.
- **Cooked log + ratings:** ? — same.
- **Shopping list:** probably writer-only — it's a transient working artefact, not interesting publicly.

Lock these in before RLS is written.

### 6.4 Repo and naming hygiene
- Pick a name for the app and use it consistently (URL, repo, README, UI).
- Decent commit messages from the first commit.
- README that explains what this is.
- No keys, tokens, or secrets in the repo.
- Public repo from day one — easier than flipping it later.

### 6.5 Build journal
Keep notes as you build: decisions made, what Lovable handled well, where it broke down, where you escalated to Claude Code, prompts that worked, prompts that didn't. The story of building this with AI *is* the portfolio asset. Trivial to keep going; near-impossible to reconstruct after the fact.

---

## 7. Open Questions

- **Import schema design.** Pending — Mike to move a couple of existing recipes from prior Claude chats into the project so the schema can be designed against real data.
- **Per-table public-read decisions.** Per §6.3 — needs locking before RLS is written.
